if (typeof FasterTabBrowsing == "undefined") {
  let FasterTabBrowsing = function() {
    ...
    let tabIsFavorite = function(tab) {
      return tab.hasAttributeNS(NS, FAVORITETAB);
    }
    ...
    function toggleTabFavorite(tab) {
      if (tabIsFavorite(tab)) {
        ...
        favPos = storage.favTabs.indexOf(tab.tabindex);
        if (favPos > -1) {
    ...
    exports.onUnload = function (reason) {
      ...
      // Note: "uninstall" doesn't ever happen currently, bug #627432.
      // Note2: "shutdown" gives error with the clearing function for
      // some reason when closing by pressing x-icon (not by ctrk-q).
    }
    
    // Fires when ctrl+q is pressed (quits application).
    require("sdk/system/events").on("quit-application-requested", function() {
      clearAndUpdateFavsToStorage();
    });
    // Fires when a single windows is closed by pressing the x-icon.
